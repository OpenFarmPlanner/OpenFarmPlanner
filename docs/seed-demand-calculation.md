...
1. **Per-plan requirement.** Pick the seed rate for the plan's
   ...
   Agent API writes should use the `seed_requirements` object on direct culture
   create/update endpoints instead of the flat `seed_rate_*` fields. It is a
   normalized write alias for these per-cultivation seed rates, not a total
   purchase requirement: `value` plus `unit` describes seed per square metre,
   row metre, or target plant. The total seed demand is derived here later from
   the planting plans' area or plant count.
