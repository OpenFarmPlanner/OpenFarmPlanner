...
type CropsPublishConfirmDialogProps = {
  open: boolean;
  cropName: string;
  accepted: boolean;
...
/**
 * Presentational confirmation dialog for publishing a crop to the public
 * library. State and the publish handler live in Crops.tsx.
 */
export function CropsPublishConfirmDialog({
  open,
  cropName,
  accepted,
  onAcceptedChange,
  ...
  onConfirm,
  t,
}: CropsPublishConfirmDialogProps) {
  return (
    <Dialog
      open={open}
      onClose={onClose}
      ...
      <DialogContent sx={{ pt: 1 }}>
        <Stack spacing={1.5}>
          <Typography color="text.secondary">
            {t('library.publishConfirm.intro', { name: cropName })}
          </Typography>
          <Stack spacing={1}>
