...
test('keeps the Kultur header actions top-right at every viewport', async ({ page, request }, testInfo) => {
  await loginWithDeterministicProject(page, request, `crop-header-${testInfo.workerIndex}`);
  ...
  for (const name of [SHORT_NAME, LONG_NAME]) {
    const crop = await apiRequest<{ id: number }>(page, 'POST', '/crops/', { name });
    cropIds[name] = crop.id;
  }
  ...
  for (const viewport of VIEWPORTS) {
    ...
    for (const name of [SHORT_NAME, LONG_NAME]) {
      await page.goto(`/app/crops?cropId=${cropIds[name]}`);
      await waitForPageStable(page, /Kulturbibliothek/i);

      const header = page.getByTestId('crop-detail-header');
      await expect(header).toBeVisible();
      const title = header.locator('h2, [data-testid="crop-title-selector-label"]').first();
