"""Execution of a previously previewed culture import.
...
from farm.models import CultureImportDraft
...
# existing CultureSerializer exposes. The agent-facing contract stays in SI
...
def build_serializer_payload(item: dict[str, Any]) -> dict[str, Any]:
    """Translate one analyzed row into a CultureSerializer payload.
    ...
    :return: Payload dictionary for :class:`CultureSerializer`.
    """
...
def _apply_row(item: dict[str, Any], *, project, context: dict[str, Any]) -> dict[str, Any]:
    """Create or update the culture for one analyzed row."""
    ...
    if action == ACTION_CREATE:
        serializer = CultureSerializer(data=payload, context=context)
        serializer.is_valid(raise_exception=True)
        # `project` is read-only on the serializer and assigned here from the
        # token's bound project, so a payload can never place a culture in a
        # different project.
        culture = serializer.save(project=project)
        return {'index': item['index'], 'action': ACTION_CREATE, 'culture_id': culture.id}

    existing = Culture.objects.filter(project=project, pk=item['matched_culture_id']).first()
    if existing is None:
        raise ImportExecutionError(
            'match_disappeared',
            f"The culture matched for row {item['index']} no longer exists. Re-run the preview.",
        )
    serializer = CultureSerializer(existing, data=payload, partial=True, context=context)
    serializer.is_valid(raise_exception=True)
    culture = serializer.save()
    return {'index': item['index'], 'action': ACTION_UPDATE, 'culture_id': culture.id}


def _check_draft_is_executable(
    draft: CultureImportDraft,
    *,
    checksum: str,
    confirmed: bool,
...
def apply_import_draft(
    ...
    *,
    ...
    """
    if draft.status == CultureImportDraft.STATUS_APPLIED:
        ...
        for item in draft.preview['items']:
            if item['action'] == ACTION_SKIP:
                rows.append({
                    'index': item['index'],
                    'action': ACTION_SKIP,
                    'culture_id': item['matched_culture_id'],
                })
                continue
            try:
    ...
    draft.status = CultureImportDraft.STATUS_APPLIED
