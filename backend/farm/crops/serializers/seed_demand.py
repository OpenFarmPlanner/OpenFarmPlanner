...
class SeedDemandSerializer(serializers.Serializer):
    """Read-only serializer for aggregated seed demand per crop."""
    crop_id = serializers.IntegerField()
    crop_name = serializers.CharField()
    crop_display_name = serializers.CharField(allow_blank=True, allow_null=True)
    crop_display_language_code = serializers.CharField(allow_blank=True)
    ...
    calculation_blockers = serializers.ListField(
    ...
    package_blocker = serializers.CharField(allow_null=True, required=False)
