"""Unit tests for culture-import unit handling, conversion, and plausibility."""

from django.test import TestCase

from farm.models import Culture, Project, Supplier
from farm.services.culture_import.analysis import (
    ACTION_BLOCKED,
    ACTION_CREATE,
    analyze_import_payload,
)
from farm.services.culture_import.units import (
...
class UnknownFieldTests(ImportAnalysisTestCase):
    """Order-form columns must not be folded into culture master data."""
...
class MatchingAndDuplicateTests(ImportAnalysisTestCase):
    ...
    def test_existing_culture_is_matched_and_marked_as_update(self):
        Culture.objects.create(name='Brokkoli', variety='Calabrese', project=self.project)
        preview = self.analyze({'name': 'Brokkoli', 'variety': 'Calabrese', 'row_spacing_cm': 50})
        row = preview['items'][0]

        self.assertEqual(row['action'], 'update')
        self.assertIsNotNone(row['matched_culture_id'])
    ...
    def test_cultures_of_other_projects_are_never_matched(self):
        other_project = Project.objects.create(name='Other', slug='other-project')
        Culture.objects.create(name='Brokkoli', variety='Calabrese', project=other_project)

        preview = self.analyze({'name': 'Brokkoli', 'variety': 'Calabrese'})

        self.assertEqual(preview['items'][0]['action'], ACTION_CREATE)
        self.assertIsNone(preview['items'][0]['matched_culture_id'])

    def test_supplier_specific_culture_is_not_matched_without_a_supplier(self):
    ...
    def test_supplier_name_matches_the_supplier_specific_culture(self):
        supplier = Supplier.objects.create(
            name='Bingenheimer', homepage_url='https://bingenheimer.example', project=self.project
        )
        ...
        self.assertEqual(preview['items'][0]['matched_culture_id'], existing.id)

    def test_two_rows_for_the_same_culture_block_the_second(self):
        preview = self.analyze(
...
class PreviewSummaryTests(ImportAnalysisTestCase):
    ...
    def test_summary_counts_each_action(self):
        Culture.objects.create(name='Bestand', project=self.project, row_spacing_m=0.5)
        preview = self.analyze(
            {'name': 'Neu'},
