...
class StructuredDiscussionMigrationTest(TransactionTestCase):
    ...
    def setUp(self) -> None:
        ...
        PublicCrop = old_apps.get_model('farm', 'PublicCulture')
        Comment = old_apps.get_model('farm', 'PublicCultureDiscussionComment')
        author = User.objects.create(username='legacy-author')
        crop = PublicCrop.objects.create(name='Tomato', variety='Roma', status='published')
        first = Comment.objects.create(public_culture=crop, created_by=author, body='First legacy comment')
        second = Comment.objects.create(public_culture=crop, created_by=author, body='Second legacy comment')
