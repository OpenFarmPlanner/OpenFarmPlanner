...
from farm.models import Culture, PublicCulture, PublicCultureTranslation
from farm.services.public_cultures import (
    detect_public_culture_duplicates,
...
class CrossLanguageDuplicateDetectionTest(ProjectApiTestCase):
    ...
    def _culture(self, **overrides) -> Culture:
        defaults = {
            'name': 'Tomate',
            'variety': 'Moneymaker',
    ...
    def test_the_english_name_of_the_same_species_is_detected_as_a_duplicate(self):
        candidate = self._culture(name='Tomato', crop_species=self.species)

        duplicates = detect_public_culture_duplicates(candidate)
    ...
    def test_the_german_name_of_the_same_species_is_detected_as_a_duplicate(self):
        candidate = self._culture(name='Tomate', crop_species=self.species)
    ...
    def test_resolves_the_species_from_the_name_when_none_is_linked(self):
        # No crop_species on the culture: the English name must still resolve
        # to the shared species and find the German entry.
        candidate = self._culture(name='Tomato')
    ...
    def test_a_different_variety_of_the_same_species_is_not_a_duplicate(self):
        candidate = self._culture(name='Tomato', variety='Ochsenherz', crop_species=self.species)
    ...
    def test_variety_comparison_ignores_casing_and_surrounding_whitespace(self):
        for variety in ('moneymaker', '  Moneymaker  ', 'MONEYMAKER', 'Money  maker'):
            candidate = self._culture(name='Tomato', variety=variety, crop_species=self.species)
    ...
    def test_normalization_never_rewrites_the_stored_variety_name(self):
        ...
        detect_public_culture_duplicates(candidate)
    ...
    def test_a_different_species_is_not_a_duplicate(self):
        ...
        candidate = self._culture(name='Karotte', crop_species=carrot)
    ...
    def test_legacy_entries_without_a_species_are_still_matched_by_name(self):
        ...
        candidate = self._culture(name='Pastinake', variety='Halblange')

        self.assertIn(legacy.id, [item.id for item in detect_public_culture_duplicates(candidate)])


class PublicCultureTranslationTest(ProjectApiTestCase):
    def setUp(self):
        super().setUp()
        self.public_culture = PublicCulture.objects.create(
            name='Tomate',
            variety='Moneymaker',
    ...
    def test_a_language_may_appear_only_once_per_entry(self):
        ...
        PublicCultureTranslation.objects.create(
            public_culture=self.public_culture, language_code='de', description='Erste',
        )

        with self.assertRaises(IntegrityError):
            with transaction.atomic():
                PublicCultureTranslation.objects.create(
                    public_culture=self.public_culture, language_code='de', description='Zweite',
                )
    ...
    def test_localized_description_falls_back_and_reports_the_language(self):
        ...
        self.assertEqual(
            self.public_culture.localized_description('en'),
    ...
    def test_editing_the_original_language_keeps_notes_in_step(self):
        ...
        self.public_culture.refresh_from_db()
    ...
    def test_unsupported_language_codes_are_ignored_not_stored(self):
        ...
        self.assertNotIn('fr', self.public_culture.descriptions_by_language())
